...
df['ILE'] = df['ata'] + df['atc'] + df['att']
df['AUA/ILE'] = df['ata'].div(df.ILE, axis=0)

# Calculate variance (σ²)
variance = df['AUA/ILE'].var()

# Calculate standard deviation (σ)
std_dev = df['AUA/ILE'].std()

#df.to_csv(tsv_output, sep = '\t', mode='w',index=False, header=not os.path.isfile(tsv_output))

print(bname + "\t" + str(variance) + "\t" + str(std_dev))
# END.
